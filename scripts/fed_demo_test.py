...
class VFLClient():
    ...
    def create_optimiser(self, learning_rate):
        if self.optimiser is None:
            self.optimiser = torch.optim.Adam(self.model.parameters(), lr=learning_rate, weight_decay=1e-4)
...
class VFLServer():
    def __init__(self, data):
        ...
        # )
        self.optimizer = optim.Adam(self.model.parameters(), lr=0.01, weight_decay=1e-4)  # optim.SGD(self.model.parameters(), lr=0.01)
    ...
    def sample_data(self):
        data_sample = self.data.sample(3000)
    ...
    def get_gradients(self, results):
        ...
        grads = self.embeddings.grad.split([4]*3, dim=1)
        # Converts to numpy.
...
def main():
    ...
    datac1 = pd.read_csv('../python/vfl-train-demo/datasets/clientoneData.csv')
    datac2 = pd.read_csv('../python/vfl-train-demo/datasets/clienttwoData.csv')
    datac3 = pd.read_csv('../python/vfl-train-demo/datasets/clientthreeData.csv')
    datas = pd.read_csv('../python/vfl-train-model-demo/datasets/outcomeData.csv')
