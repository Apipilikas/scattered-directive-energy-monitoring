...
# Relative energy change (%) compared with the synchronous VFL baseline.
methods = ["FedBCD", "Overlap-FedBCD", "FedEncrypt"]
environments = ["Local", "FABRIC"]

changes = {
    "Local":  [-19.32, -31.06, 1412.58],
    "FABRIC": [-84.36, -83.05, 1460.84],
}
...
ax.set_ylabel("Energy change (%)", fontsize=7)
...
# Standard log scales cannot display negative values.
# symlog is logarithmic away from zero and linear around zero.
...
output_path = Path(__file__).with_name("relative_energy_change_2col.png")
